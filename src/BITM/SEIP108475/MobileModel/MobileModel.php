...
namespace App\BITM\SEIP108475\MobileModel;
...
//         print_r($arr);
    ...
    public function store() {
        ...
        $query = "INSERT INTO `atomic_project`.`mobilemodel` (`id`, `name`, `model`) VALUES (NULL, '$this->name','$this->model')";
    ...
    public function show($id = '') {
        ...
        $con = mysql_connect("localhost", "root")or die("Mysql not connected");
        $dd = mysql_select_db('atomic_project');
        $q = "SELECT * FROM `mobilemodel` where `id`='$id'";
...
//        Utility::dd($result);
    ...
    public function update($id = '') {
        ...
        $conn = mysql_connect("localhost", "root")or die("Mysql not connected");
        $db = mysql_select_db('atomic_project');
        $query = "UPDATE `atomic_project`.`mobilemodel` SET `name` = '$this->name', `model` = '$this->model' WHERE `mobilemodel`.`id` = $this->id;";
//        Utility::p($query);
//        Utility::d($query);
        if (mysql_query($query)) {
    ...
    public function delete($id = '') {
        ...
        $con = mysql_connect("localhost", "root")or die("Mysql not connected");
        $dd = mysql_select_db('atomic_project');
        $q = "DELETE FROM `atomic_project`.`mobilemodel` WHERE `mobilemodel`.`id` = $id";
