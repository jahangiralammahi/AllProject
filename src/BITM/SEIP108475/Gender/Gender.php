...
namespace App\BITM\SEIP108475\Gender;
...
//         print_r($arr);
    ...
    public function store() {
        $condition=implode("", $this->name);
        $conn = mysql_connect("localhost", "root")or die("Mysql not connected");
        $db = mysql_select_db('atomic_project');
        $query = "INSERT INTO `atomic_project`.`gender` (`id`,`gender`) VALUES (NULL,'$condition')";
    ...
    public function show($id = '') {
        ...
        $con = mysql_connect("localhost", "root")or die("Mysql not connected");
        $dd = mysql_select_db('atomic_project');
        $q = "SELECT * FROM `gender` where `id`='$id'";
...
//        Utility::dd($result);
    ...
    public function update($id = '') {
        ...
        $conn = mysql_connect("localhost", "root")or die("Mysql not connected");
        $db = mysql_select_db('atomic_project');
        $query = "UPDATE `atomic_project`.`gender` SET  `gender` = '$this->name' WHERE `gender`.`id` = $this->id;";
...
//        Utility::d($query);
    ...
    public function delete($id = '') {
        ...
        $con = mysql_connect("localhost", "root")or die("Mysql not connected");
        $dd = mysql_select_db('atomic_project');
        $q = "DELETE FROM `atomic_project`.`gender` WHERE `gender`.`id` = $id";
